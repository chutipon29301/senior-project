...
def getUniPrice(k, buyers, sellers):
    buyer_acc_q = np.cumsum([buyer.quantityWant for buyer in buyers])
    seller_acc_q = np.cumsum([seller.quantityAvailable for seller in sellers])

    buyer_line = [Point(buyer_acc_q[i], buyer.bidPrice) for i, buyer in enumerate(buyers)]
    seller_line = [Point(seller_acc_q[i], seller.reservePrice) for i, seller in enumerate(sellers)]
